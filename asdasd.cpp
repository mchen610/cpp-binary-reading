...
void addShips(const string &fileName, vector<Ship> &ships)
{
    ...
    for (int i = 0; i < numShips; i++)
    {
        ...
        for (int j = 0; j < ship.numWeapons; j++)
        {
            ...
            if (ship.strongestWeapon == nullptr || ship.strongestWeapon->powerRating < weapon.powerRating)
            {
...
int main()
{
    ...
    for (Ship &ship : ships)
    {
        if (strongestWeaponShip == nullptr || strongestWeaponShip != nullptr && strongestWeaponShip->strongestWeapon->powerRating < ship.strongestWeapon->powerRating)
        {
